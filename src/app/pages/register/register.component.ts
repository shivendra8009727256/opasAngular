...
export class RegisterComponent {
  ...
  constructor(private fb: FormBuilder, private ShareDataService:ShareDataService) {
    ...
    this.registerForm = this.fb.group({
      ...
      email: ['', [Validators.required, Validators.email]],
      password: ['', [Validators.required, Validators.minLength(6)]],
      country: [this.countries.find(c => c.name === 'India'), Validators.required],
  ...
  onCountryChange(event: MatSelectChange) {
    ...
    if (selectedCountryName) {
        ...
        if (selectedCountry) {
            this.selectedCountry.set(selectedCountry.name);
            this.selectedCountryCode.set(selectedCountry.dial_code);
        }
    }
  ...
  const phoneCode=  selectedCountryData ? selectedCountryData.dial_code:"";
  const obj1 = {
